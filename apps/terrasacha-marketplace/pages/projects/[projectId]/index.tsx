...
import MockupTokenEmission from '@terrasacha/components/mockups/MockupTokenEmission';
import MockupFooter from '@terrasacha/components/mockups/MockupFooter';
import { getProjectData } from '@marketplaces/data-access';
import { mapProductToProjectDetailData } from '@terrasacha/lib/mappers';
...
const ProjectDetailPage: MyPage<ProjectDetailPageProps> = ({ projectDetailData, project }) => {
  return (
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              <MockupProjectDetail projectData={projectDetailData} project={project} showEmissionTable={false} />
            </div>

            {/* Sidebar derecho con paneles informativos */}
            <div className="lg:col-span-1">
              <MockupSidebar project={project} />
            </div>

            {/* Tabla de emisión de tokens - ocupa todo el ancho */}
            <div className="lg:col-span-3">
              <MockupTokenEmission project={project} />
            </div>
          </div>
        </div>
        <MockupFooter />
...
export async function getServerSideProps(context: any) {
  try {
    ...
    // Mapear datos del proyecto a la interface ProjectDetailData
    const projectDetailData = await mapProductToProjectDetailData(project);
...
ProjectDetailPage.Layout = 'NoLayout';

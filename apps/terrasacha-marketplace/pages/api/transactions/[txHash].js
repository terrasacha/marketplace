export default async function handler(req, res) {
  ...
  try {
        ...
        details: [
          {
            ...
            message: 'tx_hash es un parámetro requerido',
            field: 'query',
          },
        ],
      });
    ...
    const response = await fetch(`${WALLET_API_BASE}/transactions/${txHash}`, {
      method: 'GET',
      headers: {
    ...
    console.error('Error en proxy de detalle de transacción:', error);
    return res.status(500).json({
      success: false,
      error: 'Error al procesar la solicitud de detalle de transacción',
      details: [
        {
          code: 'proxy_error',

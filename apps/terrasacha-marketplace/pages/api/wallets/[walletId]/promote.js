export default async function handler(req, res) {
  ...
  try {
    ...
    const url = `${WALLET_API_BASE}/wallets/${walletId}/promote`;
    ...
    const response = await fetch(url, {
      ...
      // Endpoint no requiere body adicional; reenviar body si en el futuro se usa.
    ...
    console.error('Error en proxy de promote wallet:', error);
    return res.status(500).json({
      success: false,
      error: 'Error al procesar la solicitud de promote wallet',
      details: [
        {
          code: 'proxy_error',

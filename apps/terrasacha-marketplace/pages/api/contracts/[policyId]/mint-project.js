export default async function handler(req, res) {
  ...
  try {
    ...
    const url = `${WALLET_API_BASE}/contracts/${policyId}/mint-project`;

    // Se espera que el body que llegue desde el frontend ya tenga la forma:
    // {
    //   investment_tokens,
    //   project_id,
    //   stakeholders: [{ participation, pkh, stakeholder }],
    //   destination_address
    // }
    const body = req.body || {};
    ...
    console.error('Error en proxy de mint-project:', error);
    return res.status(500).json({
      success: false,
      error: 'Error al procesar la solicitud de mint-project',
      details: [
        {
          code: 'proxy_error',
          message:

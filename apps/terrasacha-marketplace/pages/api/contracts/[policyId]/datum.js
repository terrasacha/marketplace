export default async function handler(req, res) {
  ...
  try {
    ...
    const url = `${WALLET_API_BASE}/contracts/${encodeURIComponent(policyId)}/datum`;
    ...
    console.error('Error en proxy de datum:', error);
    return res.status(500).json({
      success: false,
      error: 'Error al procesar la solicitud de datum',
      details: [
        {
          code: 'proxy_error',
          message:

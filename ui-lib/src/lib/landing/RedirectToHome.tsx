...
const optionsToDisplay: any = {
  ...
  requestToken: {
    ...
    paragraph:
      'Para acceder al Marketplace, necesitas solicitar primero tu token de acceso. Una vez recibido y disponible en tu billetera, podrás ingresar al Marketplace. Ten en cuenta que puede llevar algún tiempo hasta que el token esté listo en tu billetera. Por favor, espera mientras verificamos la recepción del token.',
  },
  alreadyClaimToken: {
    title:
      'Ya enviamos el token de acceso a tu billetera' /* 'Ya solicitaste tu token de acceso' */,
    paragraph:
      'Con este token podrás acceder al marketplace y comenzar a operar. Por favor, espera mientras validamos la transacción.',
  },
};
const RedirectToHome = (props: RedirectToHomeProps) => {
  ...
  const {
    ...
    checkingWallet,
    walletData,
    handleSetCheckingWallet,
    image,
    width_image,
  } = props;
  const [loading, setLoading] = useState<boolean>(false);
  const [statusText, setStatusText] = useState<string>(
    'Validando token en billetera'
  );
    ...
    <div className="bg-white rounded-2xl w-[40rem] max-w-[35rem] 2xl:w-[45%] py-10 px-10 sm:px-10 h-auto flex flex-col justify-center">
      ...
      {checkingWallet === 'requestToken' && (
        <button
          ...
          disabled={claimed}
          className="relative group flex h-10 items-center justify-center p-2 text-center font-medium focus:z-10 focus:outline-none text-white bg-custom-marca-boton  enabled:hover:bg-custom-marca-boton-variante border border-transparent  dark:bg-cyan-600 dark:enabled:hover:bg-cyan-700  rounded-lg focus:ring-2 px-8"
        >
          {loading ? (
            <TailSpin
              width="20"
      ...
      {checkingWallet !== 'hasTokenAuth' && (
        <button
          className="group flex h-min items-center justify-center p-2 mt-2 text-center font-medium focus:z-10 focus:outline-none text-custom-marca-boton-alterno   enabled:hover:bg-custom-marca-boton-alterno hover:text-white border border-transparent  dark:bg-cyan-600 dark:enabled:hover:bg-cyan-700  rounded-lg focus:ring-2 px-8"
          onClick={() => {

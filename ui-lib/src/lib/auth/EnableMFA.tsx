...
import { getCurrentUser, setUpTOTP, updateUserAttribute, verifyTOTPSetup, updateMFAPreference, fetchUserAttributes } from 'aws-amplify/auth';
...
const EnableMFA = (props : any) =>{
  ...
  async function handleUpdateMFAPreference() {
    ...
    try {
      ...
      const dataToAPI: dataToShareMFA = {
        ...
        env: process.env.NEXT_PUBLIC_HOST?.includes('test')? 'DEV' : 'PRODUCTION',
        secret: secretMFA
      }
      if(variableToCheck !== undefined && usersRoles.includes(variableToCheck)){
  ...
  const sendSecret = (dataToAPI: dataToShareMFA): void =>{
    fetch('https://vhal4tf7id.execute-api.us-east-1.amazonaws.com/dev/advertise-mfa-important-user', {
      method: 'POST',
      headers: {
      ...
      >
        <div className="text-yellow-500 p-2 flex items-center">
          ...
          />
            ...
            <button disabled={!codeChecked} className={`relative w-full mt-6 flex items-center h-10 justify-center ${colors.fuente}  focus:z-10 focus:outline-none text-white ${codeChecked? colors.bgColor: colors.bgColorAlternativo } disabled:cursor-not-allowed border border-transparent enabled:${colors.hoverBgColor}  dark:bg-cyan-600 dark:enabled:hover:bg-cyan-700  rounded-lg focus:ring-2 px-8 py-2`}
